...
class Network(nn.Module):
    def __init__(self):
        ...
        self.N_samples = cfg.task_arg.N_samples # number of samples per ray in coarse network 64
        self.N_importance = cfg.task_arg.N_importance # number of samples per ray in fine network 128
        self.chunk = cfg.task_arg.chunk_size # number of rays processed in parallel 4096
        self.batch_size = cfg.task_arg.N_rays # number of rays in each batch 1024
        self.white_bkgd = cfg.task_arg.white_bkgd # 用白色背景
        self.use_viewdirs = cfg.task_arg.use_viewdirs # whether use full 5D input instead of 3D
